    ...
    try {
      ...
      await fetch('/api/orders/public', {
        ...
        body: JSON.stringify({
          ...
          items: orderItems,
          total: totalEstimate || price,
          notes: sqftValue > 0 ? `${sqftValue} sqft requested` : 'Product enquiry via website',
          source: 'website',
          whatsappMessage: `Product: ${product.name}, Price: ₹${price}, Qty: ${sqftValue || 'N/A'}`,
        }),
      });
    } catch (e) {
    ...
    <div className="modal-overlay" onClick={onClose} role="dialog" aria-modal="true" aria-label={`Product details: ${product.name}`}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-info">
          ...
          <button onClick={handleWhatsApp} disabled={saving} className="btn-primary modal-cta" style={{ cursor: saving ? 'wait' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
            ...
            {saving ? 'Saving...' : sqftValue > 0 ? `WhatsApp - ₹ ${totalEstimate.toLocaleString()}` : 'WhatsApp / Get Price'}
          </button>
        </div>
      </div>
    </div>
